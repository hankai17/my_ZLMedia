...
void FlvPlayer::onPlayResult_l(const SockException &ex , bool handshakeCompleted) {
    ...
    if(!ex) {
        ...
        weak_ptr<FlvPlayer> weakSelf = dynamic_pointer_cast<FlvPlayer>(shared_from_this());
        ...
        _pMediaTimer.reset( new Timer(timeoutMS / 2000.0, [weakSelf,timeoutMS]() {
            ...
            if(strongSelf->_mediaTicker.elapsedTime()> timeoutMS) {
                //接收flv媒体数据超时
                strongSelf->onPlayResult_l(SockException(Err_timeout,"receive flv timeout"),true);
                return false;
            }
            return true;
        },getPoller()));
...
void FlvPlayer::play(const string& strUrl) {
    ...
    if (!_strApp.size() || !_strStream.size()) {
        onPlayResult_l(SockException(Err_other,"flv url非法"),false);
        return;
    }
